...
def to_indices_with_extracted_entities(graphs, word2idx):
    ...
    :param word2idx:
    :return:
        sentences_matrix[graph(sentence)_index, edge(relation)_index] = property_kbid (relation_type)
    """
    max_sent_len = model_params['max_sent_len']  #36
    sentences_matrix = np.zeros((len(graphs), max_sent_len), dtype="int32")  # (sentence_number, sentence_len)
    entity_matrix = np.zeros((len(graphs), MAX_EDGES_PER_GRAPH, max_sent_len), dtype="int8")  # (sentence_number, max_edge, sentence_len)
    y_matrix = np.zeros((len(graphs), MAX_EDGES_PER_GRAPH), dtype="int16")  # (sentence_number, max_edge)

    for index, g in enumerate(tqdm.tqdm(graphs, ascii=True)):

        token_sent_ids = embeddings.get_idx_sequence(g["tokens"], word2idx)

        # TODO: length of tokens should be less or equal than max_sent_len. (this can be deleted.)
        if len(token_sent_ids) > max_sent_len:
            token_sent_ids = token_sent_ids[:max_sent_len]

        # sentences_matrix[index, :len(token_sent_ids)] = token_sent_ids
        sentences_matrix[index, :] = token_sent_ids
